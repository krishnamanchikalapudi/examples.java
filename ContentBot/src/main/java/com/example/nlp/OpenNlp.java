...
public class OpenNlp {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException, InterruptedException {
		...
		OpenNlp chatBot = new OpenNlp();
		// Train categorizer model to the training data we created.
		DoccatModel model = chatBot.trainCategorizerModel();
		...
		while (true) {
			...
			String[] sentences = chatBot.breakSentences(userInput);
			...
			for (String sentence : sentences) {
				...
				String[] tokens = chatBot.tokenizeSentence(sentence);

				// Tag separated words with POS tags to understand their gramatical structure.
				String[] posTags = chatBot.detectPOSTags(tokens);

				// Lemmatize each word so that its easy to categorize.
				String[] lemmas = chatBot.lemmatizeTokens(tokens, posTags);

				// Determine BEST category using lemmatized tokens used a mode that we trained
				// at start.
				String category = chatBot.detectCategory(model, lemmas);
